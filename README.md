...
Projects stay shared at `/srv/ccc/projects`. Setup CCC Profile adds the user to `ccc`, grants the shared `ccc` group read/traverse access on that managed home so the dashboard file browser can list it, links `~/projects`, syncs Claude/Codex/Gemini config and skills into that user's home from `oculus-configs` via `ccc-sync-agent-configs`, installs the Claude settings/statusline baseline, installs that user's own Claude Code CLI into `~/.local/bin` (Codex and Gemini CLIs are shared from `/usr/local/ccc-npm`), validates the provider CLIs are reachable, and repairs shared project permissions. The managed GitHub machine key lives at `/etc/ccc/ssh/github_ed25519`; the GitHub page can generate it, copy the public key, test SSH access, configure work identities, or explicitly promote an existing current-user key. Provider auth tokens are not copied between users. After setup, sign out and back in as the work identity so the new `ccc` group membership is active, then run `claude`, `codex`, `gemini`, and optionally `gh auth login`.

Shell environment, PATH, aliases, and the login-to-`~/projects` helper are machine-wide (`/etc/profile.d/ccc-env.sh` + `/etc/ccc/ccc-shell.sh`), so every account gets them without per-user `.bashrc` changes.
...
`ccc-sync-agent-configs` keeps `/opt/oculus-configs` as a shared root-owned checkout and is the single sync path: it refreshes `oculus-configs`, copies known Claude/Codex/Gemini files and directories into the target home, writes the CCC-managed Claude `settings.json` (with `permissions.defaultMode: bypassPermissions`) and statusline script when missing, merges rather than overwrites existing settings, and chowns touched directories. The GUI's Accounts > Sync Account Configs, Setup CCC Profile, and Sync All Account Configs all shell out to this same installed script (`--user <name>` / `--all-users`) instead of carrying their own copy. Provider profiles are never mirrored between accounts — each account owns its `.claude`/`.codex`/`.gemini` state (auth, sessions, history, UI prefs); only the `oculus-configs` baseline is shared. Provider Configs shows the managed files plus the synced rules/skills/template directories.
