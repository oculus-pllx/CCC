...
func directAgentConfigSyncScript(runLine string) string {
	return strings.Join([]string{
		...
		`  [ -f "$from" ] || { printf 'missing source file %s\n' "$from"; return 1; }`,
		`  install -D -m 0644 "$from" "$to"`,
		`  printf '  copied file %s\n' "$label"`,
		"}",
		"copy_dir() {",
		`  local from=$1 to=$2 label=$3`,
		`  [ -d "$from" ] || { printf 'missing source dir %s\n' "$from"; return 1; }`,
		`  rm -rf "$to"`,
		`  mkdir -p "$to"`,
		`  cp -a "$from"/. "$to"/`,
		`  printf '  copied dir  %s\n' "$label"`,
		...
		"copy_optional_dir() {",
		`  local from=$1 to=$2 label=$3`,
		`  if [ ! -d "$from" ]; then printf '  default dir missing, skipped %s\n' "$label"; return 0; fi`,
		`  rm -rf "$to"`,
		`  mkdir -p "$to"`,
		`  cp -a "$from"/. "$to"/`,
		`  printf '  copied default dir %s\n' "$label"`,
		"}",
		...
		`#!/bin/bash`,
		`echo "${USER}@$(hostname -s):$(pwd | sed "s|^$HOME|~|") [claude] $(date +"%I:%M%P" | sed 's/^0//')"`,
		`CLAUDESTATUSLINE`,
		`  chmod +x "$home/.claude/bin/statusline-command.sh"`,
		`  printf '  wrote Claude settings/statusline\n'`,
		"}",
